...
class TimelineRecorder {
  ...
  async loadTimeline() {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== "object") {
        throw new Error("Timeline must be a valid JSON object");
      }
      if (typeof parsed.scenario_name !== "string") {
        throw new Error("Timeline must have a string scenario_name property");
      }
      if (
        typeof parsed.duration_seconds !== "number" ||
        !isFinite(parsed.duration_seconds)
      ) {
        throw new Error(
          "Timeline must have a finite number duration_seconds property"
        );
      }
      if (!Array.isArray(parsed.timeline)) {
        throw new Error("Timeline must have an array timeline property");
      }
  ...
  async getChromePath() {
    ...
    throw new Error(
      "Chrome/Chromium not found. Please install or set CHROME_PATH"
    );
  }
  ...
  async setupRecorder() {
    ...
    const outDir = path.join(process.cwd(), "demo_videos");
  ...
  async executeAction(event) {
    ...
    console.log(
      `\n⏱️  ${elapsedSeconds}s | ${event.action}: ${
        event.narration || event.target
      }`
    );
  ...
  getSelector(target) {
    ...
    const selectorMap = {
      ...
      impact_card:
        '[data-testid="impact-card"], .impact-card, article:first-of-type',
      impact_card_expand: "button:first-of-type",
      news_tab: '[role="tab"][data-value="news"]',
      analysis_tab: '[role="tab"][data-value="analysis"]',
      actions_tab: '[role="tab"][data-value="actions"]',
    ...
    return await this.page.evaluateHandle(
      (sel, txt) => {
        ...
        for (const element of elements) {
          if (element.textContent.trim() === txt) {
            return element;
          }
        }
        return null;
