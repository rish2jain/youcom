...
class EnhancedDemoRecorder {
  ...
  async highlightElement(selector) {
    try {
      const timerId = await this.page.evaluateHandle((sel) => {
        const element = document.querySelector(sel);
        if (element) {
      ...
      // Clean up timer on page navigation/close
      const cleanup = async () => {
        try {
          if (timerId) {
            await this.page.evaluate(
              (id) => clearTimeout(id),
              await timerId.jsonValue()
            );
          }
        } catch (e) {
      ...
      this.page.once("framenavigated", cleanup);
    ...
    while (this.recording) {
      ...
      if (elapsed >= targetFrameTime) {
        try {
          ...
          console.error(
            `⚠️ Screenshot capture failed (frame ${this.frameCount}):`,
            error.message
          );
      ...
      const nextWait = Math.max(
        5,
        targetFrameTime - (Date.now() - lastFrameTime)
      );
  ...
  async quickDemo() {
    await this.addNarration(
      "Enterprise CIA: Competitive Intelligence Automation"
    );
    await this.wait(2);

    await this.addNarration(
      "Real-time threat monitoring with all 4 You.com APIs"
    );
    await this.scrollBy(300, 2);
    await this.wait(2);
  ...
  async customDemo(duration) {
    ...
    for (let i = 0; i < segments; i++) {
      await this.addNarration(
        `Demonstrating Enterprise CIA features - segment ${i + 1}`
      );
  ...
  async cleanup() {
    if (this.cleanupScreenshots) {
      try {
        ...
        await Promise.all(
          files.map((file) =>
            fs.promises.unlink(path.join(this.screenshotDir, file))
          )
        );

        // Remove directory
        await fs.promises.rm(this.screenshotDir, {
          recursive: true,
          force: true,
        });
  ...
  async record() {
    try {
      ...
      console.log(
        `   Mode: ${this.mode} | Quality: ${this.quality} | FPS: ${this.fps}\n`
      );
      ...
      const success = await this.navigateTo(
        this.baseUrl,
      ...
      if (this.mode === "quick") {
        ...
        await this.customDemo(this.duration);
      }
