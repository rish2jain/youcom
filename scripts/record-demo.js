 ...
 *   --url <url>                    Custom URL to record (default: localhost:3456)
...
class EnterpriseCIARecorder {
  constructor(options = {}) {
    ...
    // Quality presets
    this.qualityPresets = {
      low: { fps: 10, crf: 28, preset: "faster", resolution: { width: 1280, height: 720 } },
      medium: { fps: 15, crf: 23, preset: "medium", resolution: { width: 1280, height: 720 } },
      high: { fps: 30, crf: 18, preset: "slow", resolution: { width: 1920, height: 1080 } },
    };
  ...
  async initialize() {
    console.log(`\n🚀 Enterprise CIA Demo Recorder`);
    console.log(`   Mode: ${this.mode} | Quality: ${this.quality} | Duration: ${this.duration}s`);
      ...
      if (this.isRecording && this.page) {
        try {
          ...
          if (this.screenshotCount % (this.fps * 5) === 0) {
            ...
            console.log(`📸 Recording: ${elapsed}s / ${this.duration}s (${progress}%)`);
          }
        } catch (error) {
  ...
  async stopRecording() {
    if (this.recordingInterval) {
      ...
      console.log(`📸 Captured ${this.screenshotCount} frames (${totalSeconds} seconds)`);
  ...
  async createVideo() {
    ...
    const ffmpegArgs = [
      ...
      "-i", path.join(this.screenshotsDir, "frame_%06d.png"),
      "-c:v", "libx264",
      "-preset", this.ffmpegPreset,
      "-crf", this.crf.toString(),
      "-pix_fmt", "yuv420p",
      "-vf", `scale=${this.resolution.width}:${this.resolution.height}`,
      "-y",
      this.videoPath,
    ];
      ...
      ffmpeg.stderr.on("data", (data) => {
        ...
        if (message.includes("frame=")) {
          ...
          if (frameMatch) {
            ...
            const progress = Math.floor((currentFrame / this.screenshotCount) * 100);
  ...
  async saveNarrationMarkers() {
    ...
    const content = this.narrationMarkers
      .map(({ timestamp, text, elapsed }) => `[${elapsed}s] ${timestamp}: ${text}`)
      .join("\n");
  ...
  async quickDemo() {
    ...
    await this.addNarration("Welcome to Enterprise CIA - Competitive Intelligence Automated");
    await this.wait(5);

    await this.addNarration("Powered by all four You.com APIs working together");
    await this.wait(8);
  ...
  async fullDemo() {
    ...
    if (clicked) {
      await this.addNarration("Demonstrating seamless navigation and API integration");
      await this.wait(10);
    }
  ...
  async run() {
    ...
    try {
      ...
      const loaded = await this.navigateTo(this.baseUrl, "Enterprise CIA Dashboard");
      if (!loaded) {
