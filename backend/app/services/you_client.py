...
class YouComOrchestrator:
    ...
    def _build_explainability(
        ...
        """Build basic explainability data - enhanced version will be generated by ExplainabilityEngine"""
        return {
            "reasoning": analysis.get("reasoning"),
            "impact_areas": analysis.get("impact_areas", []),
            "key_insights": analysis.get("key_insights", []),
            "source_summary": source_quality,
            # Placeholder for enhanced explainability - will be populated by ExplainabilityEngine
...
Format the response with clear sections and actionable insights."""
    ...
    async def generate_impact_card(
        ...
        *,
        ...
        try:
            ...
            # Step 5: Generate Decision Engine Recommendations
            logger.info("🎯 Step 5: Generating action recommendations...")
            decision_recommendations = []
            if db_session:
                try:
                    from app.services.decision_engine import DecisionEngine
                    from app.schemas.action_recommendation import DecisionEngineRequest
                    
                    decision_engine = DecisionEngine(db_session)
                    
                    # Create request for Decision Engine
                    decision_request = DecisionEngineRequest(
                        risk_score=analysis_data.get("analysis", {}).get("risk_score", 0),
                        competitor_name=competitor,
                        impact_areas=analysis_data.get("analysis", {}).get("impact_areas", []),
                        key_insights=analysis_data.get("analysis", {}).get("key_insights", []),
                        confidence_score=analysis_data.get("analysis", {}).get("confidence_score", 0),
                        context={
                            "sources": news_data.get("articles", [])[:3],  # Top 3 news sources
                            "search_results": context_data.get("results", [])[:2]  # Top 2 search results
                        }
                    )
                    
                    # Generate recommendations
                    decision_response = await decision_engine.generate_recommendations(decision_request)
                    decision_recommendations = [rec.dict() for rec in decision_response.recommendations]
                    
                    logger.info(f"✅ Generated {len(decision_recommendations)} action recommendations")
                    
                except Exception as e:
                    logger.warning(f"⚠️ Decision Engine failed, continuing without recommendations: {str(e)}")
            
            # Step 6: Assemble Impact Card
            logger.info("🎯 Step 6: Assembling Impact Card...")
    ...
    def assemble_impact_card(self, news_data: Dict, context_data: Dict, analysis_data: Dict, research_data: Dict, competitor: str, decision_recommendations: List[Dict] = None) -> Dict[str, Any]:
        ...
        # Use Decision Engine recommendations if available, otherwise fall back to enriched actions
        if decision_recommendations:
            recommended_actions = decision_recommendations
            next_steps_plan = decision_recommendations
        else:
