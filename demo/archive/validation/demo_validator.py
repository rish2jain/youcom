...
class DemoValidator:
    ...
    def _check_detection(self, result: IncidentStateModel) -> None:
        ...
        rec = getattr(result.detection, 'primary_recommendation', None) if result.detection else None
        if not rec:
            self.checks.append(
                ValidationCheck(
        ...
        elif not hasattr(rec, 'confidence') or not isinstance(getattr(rec, 'confidence', None), (int, float)):
            self.checks.append(
                ValidationCheck(
                    name="detection_confidence",
                    passed=False,
                    message="Detection recommendation missing or invalid confidence",
                    severity="error",
                )
            )
    ...
    def _check_prediction(self, result: IncidentStateModel) -> None:
        ...
        rec = getattr(result.prediction, 'primary_recommendation', None) if result.prediction else None
        if rec and hasattr(rec, 'parameters'):
            parameters = getattr(rec, 'parameters', None)
            if isinstance(parameters, dict) and "projected_cost" in parameters:
                cost = parameters["projected_cost"]
                if isinstance(cost, (int, float)):
                    ...
                    self.checks.append(
                        ValidationCheck(
                            ...
                            passed=False,
                            message="Prediction cost is not numeric",
                            severity="warning",
                        )
                    )
            else:
    ...
    def _check_consensus(self, result: IncidentStateModel) -> None:
        ...
        if not hasattr(result.consensus, 'final_confidence'):
            self.checks.append(
                ValidationCheck(
                    name="consensus_confidence",
                    passed=False,
                    message="Consensus missing final confidence",
                    severity="error",
                )
            )
            return
        ...
        if not isinstance(confidence, (int, float)):
            self.checks.append(
                ValidationCheck(
                    name="consensus_confidence",
                    passed=False,
                    message="Consensus confidence is not numeric",
                    severity="error",
                )
            )
        elif confidence < 0.5:
    ...
    def _check_communication(self, result: IncidentStateModel) -> None:
        ...
        rec = getattr(result.communication, 'primary_recommendation', None) if result.communication else None
        params = getattr(rec, 'parameters', None) if rec else None
        if rec and isinstance(params, dict) and "summary" in params:
            self.checks.append(
                ValidationCheck(
                    name="communication_phase",
                    passed=True,
    ...
    def _check_timeline_consistency(self, result: IncidentStateModel) -> None:
        ...
        if malformed_events > 0:
            self.checks.append(
                ValidationCheck(
                    name="timeline_malformed",
                    passed=False,
                    message=f"{malformed_events} timeline events missing phase attribute",
                    severity="warning",
                )
            )
