...
class SystemLevelRecorder {
  constructor(config = {}) {
    ...
    // Set defaults first, then apply user config
    const defaults = {
      videoDir: "demo_videos",
  ...
  async startRecording() {
    ...
    if (this.platform === "darwin") {
      ...
      ffmpegArgs = [
        ...
        "1:", // Screen 1, no audio
        "-vf",
        `crop=${this.windowInfo.width}:${this.windowInfo.height}:${this.windowInfo.x}:${this.windowInfo.y}`,
        "-c:v",
        "libx264",
        "-preset",
        quality.preset,
      ...
      ffmpegArgs = [
        ...
        this.config.fps.toString(),
        "-video_size",
        `${this.windowInfo.width}x${this.windowInfo.height}`,
        "-i",
        `${display}+${this.windowInfo.x},${this.windowInfo.y}`,
        "-c:v",
        "libx264",
        "-preset",
        quality.preset,
    ...
    return new Promise((resolve, reject) => {
      ...
      const cleanup = () => {
        ...
        this.recordingProcess.removeAllListeners("error");
        this.recordingProcess.removeAllListeners("exit");
      };

      // Handle early exit or error
  ...
  async runDemoSequence() {
    ...
    try {
      ...
      await this.wait(
        5,
        "🎙️ Enterprise CIA - AI-Powered Competitive Intelligence"
      );
  ...
  async run() {
    ...
    if (this.platform !== "darwin" && this.platform !== "linux") {
      ...
      console.log(
        "💡 Use improved_demo_recorder.js for cross-platform support"
      );
      return;
    }
